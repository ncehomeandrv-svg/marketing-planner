...
const helperAnchor='function applyChannelOwnership(item:PlannerItem):PlannerItem';
const helperIndex=source.indexOf(helperAnchor);
if(helperIndex<0)throw new Error('Planner helper anchor not found.');
const actionIndex=source.indexOf('function ActionBucket(');
if(actionIndex>=0&&actionIndex<helperIndex){source=source.slice(0,actionIndex)+components+'\n'+source.slice(helperIndex)}
